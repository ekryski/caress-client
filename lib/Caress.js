  ...
  */
  ...
  define(function(require, exports, module) {
    ...
    Client.prototype.processCursorSet = function(packet, message){
      ...
      // Check to see if the touch is inside the document.
      // If not emit a touch cancel event and then remove it from
      // the touch list.

      var touchX = window.screenX * touch.xPosition;
      var touchY = window.screenY * touch.yPosition;

      // TODO: Deal with left edge not being zero (ie. dual monitor)
      var leftScreenEdge = 0; //window.screen.availLeft;
      var rightScreenEdge = window.screen.width; //leftScreenEdge + window.screen.availWidth;
      var topScreenEdge = 0; //window.screen.availTop
      var bottomScreenEdge = window.screen.height; //window.screen.availHeight

      // TODO: Figure out the correct top and bottom document edge
      var leftBrowserEdge = window.screenX;
      var rightBrowserEdge = rightScreenEdge - (window.screenX + window.outerWidth);
      var topDocumentEdge = window.screenY;
      var bottomDocumentEdge = bottomScreenEdge - (window.screenY + window.outerHeight);


      // If touch was left of the browser or right of the browser or above the document or below the document
      if ( (touchX >= leftScreenEdge && touchX <= leftBrowserEdge) || (touchX >= rightBrowserEdge && touchX <= rightScreenEdge) ||
            (touchY >= topScreenEdge && touchY <= topDocumentEdge) || (touchY >= bottomDocumentEdge && touchY <= bottomScreenEdge) ){

        // TODO: create touchcancel event and remove touch from list if exists
      }
